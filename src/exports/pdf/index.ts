...
import { IInvoiceDetail } from "model/invoice/response/InvoiceResponseModel";
...
export const PrintQrcode = (canvasElement, drugStore: IDrugStore, drugName: string, number: INumber, units: IUnit[]) => {
...
export const PrintBarcode = (canvasElement, drugStore: IDrugStore, drugName: string, number: INumber, units: IUnit[]) => {
  const column = [
    {
...
const printStamp = (column: any, drugName?: string, drugStore?: IDrugStore, options?: any) => {
  column = [
    {
      text: drugStore.name,
      bold: true,
  ...
  const pdf = pdfMake.createPdf({
    ...
    content: [
      {
        columns: [column, _.cloneDeep(column)],
...
export const printBill = (drugStore: IDrugStore, invoiceData: IInvoiceDetail, isPrint = true) => {
  const combos = invoiceData.invoice_detail.filter(({ drug_id }) => drug_id < 0);
  const units = invoiceData.invoice_detail.filter(
    ({ drug_id, combo_name }) => drug_id > 0 && (invoiceData.invoice.invoice_type === "IV3" || !combo_name || combo_name === "Đơn thuốc")
  );
  ...
  const pdf = pdfMake.createPdf({
    ...
    content: [
      {
        svg: process.env.APP_TYPE === "GPP" ? logoGDP : logoGPP,
        width: 50,
        alignment: "center",
      },
      {
        ...
        ? [
            ...
            [
              ...combos.reduce(
                (acc, cur) =>
                  acc.push(
                    billColumnCombo(cur.combo_name, cur.current_cost, cur.quantity, cur.current_cost * cur.quantity),
                    [...Array(89)].map(() => "·").join("")
                  ) && acc,
                []
              ),
                  ...
                  body: [
                    ...
                    ...units.reduce((acc, cur, idx) => {
                      return [
                        ...
                        [
                          {
                            text: `${idx + 1}. ` + (cur.combo_name === "Đơn thuốc" ? "* " : "") + cur.drug_name,
                            bold: true,
                            colSpan: 5,
                        ...
                        [
                          {
                            text: formatCurrency(+cur.quantity, ","),
                            bold: true,
                            alignment: "right",
                            border: [false, false, false, false],
                          ...
                          {
                            text: formatCurrency(+cur.current_cost, ","),
                            alignment: "right",
                            border: [false, false, false, false],
                          },
                          {
                          ...
                          {
                            text: formatCurrency(+cur.current_cost * +cur.quantity * (1 + +cur.vat / 100.0), ","),
                            alignment: "right",
                            bold: true,
                            border: [false, false, false, false],
                          },
    ...
    pageBreakBefore: function (currentNode, followingNodesOnPage) {
      if (currentNode.startPosition.top && Math.round(currentNode.startPosition.top) > 790 && currentNode.headlineLevel === "drugName") {
        return true;
      }
