...
export async function ExportExcel(userOptions) {
  const options = {
    title: "",
    titleExtra: [],
    titleExtraFormat: [],
  ...
  const defaultInfo = options.generateInfo
    ? [
        ["Nhà thuốc:", options.info?.drug_store.name],
        ["Địa chỉ:", options.info?.drug_store.address],
        ["Thời gian xuất:", moment().format("DD/MM/YYYY HH:mm:ss")],
        ["Người xuất:", options.info?.name],
      ]
    : [];
  ...
  const titleExtraRows = createHeaderFooter(worksheet, curRowIdx, options.titleExtra, options.titleExtraFormat, false, true, true, false);
  ...
  if (options.header && options.header.length > 0) {
    ...
    const headerRows = createHeaderFooter(worksheet, curRowIdx, options.header, options.headerFormat, true, true, true);
    curRowIdx += headerRows.length;
  }

  const headerExtraRows = createHeaderFooter(worksheet, curRowIdx, options.headerExtra, [], true, true, true, true, true);
  ...
  const dataExtraRows = createHeaderFooter(worksheet, curRowIdx, options.dataExtra, [], true, true, true, true, true);
