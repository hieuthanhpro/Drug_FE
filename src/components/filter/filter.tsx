...
function FilterDate(props: FilterItemProps) {
  ...
  return (
    <div className="filter-item filter-item--date">
      ...
      <DatePickerCustom
        value={moment(filterItem?.value?.toString(), "yyyy-MM-DD").format("DD/MM/yyyy")}
      ...
      {filterItem.type === "date-two" && (
        <Fragment>
          ...
          <DatePickerCustom
            value={moment(filterItem?.value_extra?.toString(), "yyyy-MM-DD").format("DD/MM/yyyy")}
            placeholder="DD/MM/YYYY"
            onChange={(e) =>
              handleChangeFilter({
                ...filterItem,
                value_extra: moment(e).format("yyyy-MM-DD"),
              })
            }
          />
        </Fragment>
...
export function ListFilterChoose(props: ListFilterChooseProps) {
  ...
  return (
    <>
      {listFilterItem && listFilterItem.length > 0 && listFilterItem.filter((item) => item.value || item.value === 0).length > 0 && (
              ...
              listFilterItem.map((item, index) => {
                if (item.value || item.value === 0) {
                  if (item.type !== "date" && item.type !== "date-two") {
                    ...
                    return (
                      <li key={index}>
                        {item.name}: {moment(item.value).format("DD/MM/yyyy")}
                        {item.value_extra ? ` - ${moment(item.value_extra).format("DD/MM/yyyy")}` : ""}
                        <span className="btn-remove" onClick={() => removeValueFilter(item.key)}>
                          <Icon name="Times" />
                        </span>
                      </li>
